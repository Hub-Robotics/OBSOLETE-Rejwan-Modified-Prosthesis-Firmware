...
const double theta_s0 = 4.2; //deg
const double control_command_lower_limit = 0.3; // volt full range is 0.3 to 5 v
const float k_spring = 4.0;     //Nm/deg
const float gear_ratio = 40;
const float torque_const =0.096; //0.096Nm/A Nm/A 37.0;    //mNm/A
double loop_time = 0.001;   // second
double rising_slew_rate = 5;    // 30 voltage/sec
double falling_slew_rate = -5;
...
double DesiredCurrent (double Impedance,double Angle){
    ...
    id = tau_motor/gear_ratio/torque_const;

    //id = tau_motor/gear_ratio/0.9*1000/torque_const;
    return (id);
}
