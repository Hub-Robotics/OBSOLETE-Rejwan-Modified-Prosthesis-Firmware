...
//new
uint8_t FATAL_Error,ACC_Reset_Ongoing, Skip_FATFS,File_Sensor_write_issue,File_Sensor_close_issue;
...
int FATFS_attempt,Fat_mnt_fail,FATAL_err,Fat_read_mnt_fail,Fat_write_read_fail,Fat_sync_read_fail;
...
uint8_t USB_Mode,Acc_Error_ZeroValue, Enter_Into_DFU,MSC_continue, VCP_Bypass,VCP_continue,MSC_Bypass;
int last_file;
extern int First_Imaging_after_Camera_ON, last_file_camera,Logstart_Create,Logstart_Delete,EnterDFU_Create,EnterReset_Create;
extern uint8_t Skip_I2C,Data_log_Start_Resume, ADC_Busy_with_Battery_Monitor, ErrorCodeBuf;
...
uint8_t read_conf_File(char *path)  // Read SD card content to save next value in auto increment format
{
	...
	while(1)
	{
	  ...
	  if ((strstr(fno.fname, ".AIM") != NULL) && (sscanf(fno.fname, "%d", &i) == 1))   // Searcing for .BIN file and determining highest index
		  if (i > index) index = i;
	}
	}
	return 0;
...
void FATFS_SD_Open_Ready(void)
{
  ...
  if(FATFS_LinkDriver(&SD_Driver, SDPath) == 0)    // Link SD Driver
	...
	//		 RED_LED_ON();
			 ...
			 {
			 if(f_open(&MyFile, FILE_NAME_STRING, FA_OPEN_ALWAYS | FA_WRITE) != FR_OK) // Create and Open a new .BIN file object with write access
				 {
		//		 RED_LED_ON();
				 }
				 else
...
void Store_RTC_Time(void)    // Read RTC timestamp and write in a string as hh:mm:ss format
{
  //  time Format : hh:mm:ss
	sprintf((char*)RTC_String,"%.2d:%.2d:%.2d", __RTC_CONVERT_BCD2BIN(LL_RTC_TIME_GetHour(RTC)),
...
void Store_RTC_Date(void)    // Read RTC calender and write in a string as mm-dd-yy format
{
  // date Format : mm-dd-yy
 	sprintf((char*)RTC_String,"%.2d-%.2d-%.2d", __RTC_CONVERT_BCD2BIN(LL_RTC_DATE_GetMonth(RTC)),
...
volatile uint16_t Test0,Test1,Test2,Test1_mV,Test2_mV,Test3,Test4,Test3_mV,Test4_mV,Test5,Test6,Test5_mV,Test6_mV;
...
void Write_Timestamp_In_SD (void)         // Demo RTC timestamp write in FATFS SD card for Debug
{
	...
	if(f_write(&MyFile, textString, strlen(textString), &BytesWritten1) != FR_OK)   // Write the RTC timestamp string to the file
{
		// debugging
}
...
uint8_t low_pass_filter = BITS_DLPF_CFG_188HZ; uint8_t low_pass_filter_acc = BITS_DLPF_CFG_188HZ;
...
//    gyro_angle.y = (float) last_angle.y + dt_s * (cos(last_angle.x) * gyro.y - sin(last_angle.x) * gyro.z);
    ...
    // Greg start
	accel_angle.x = (float) atan(accel.x/sqrt(pow(-accel.z,2) + pow(-accel.y,2)));
	accel_angle.y = (float) atan(accel.z/sqrt(pow(accel.x,2) + pow(-accel.y,2)));
	accel_angle.z = 0;

	gyro_angle.x  = (float) last_angle.x + dt_s * (-gyro.z + sin(last_angle.x) * tan(last_angle.y) * gyro.x+ cos(last_angle.x) * tan(last_angle.y) * -gyro.y);
	gyro_angle.y = (float) last_angle.y + dt_s * (cos(last_angle.x) * gyro.x - sin(last_angle.x) * -gyro.y);
    // Greg end
